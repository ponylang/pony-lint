...
A linter for Pony source files. Checks for style violations and reports
diagnostics with file, line, and column information.

## Status

pony-lint is pre-alpha software.
...
Create a `.pony-lint.json` file in your project root (next to `corral.json`)
to configure rule status. Each entry maps a rule ID or category name to `"on"`
or `"off"`:
...
Rule-specific entries take precedence over category entries. CLI `--disable`
flags take precedence over file configuration.
...
Inline comments let you suppress diagnostics for specific lines or regions:
...
A rule-specific `on` overrides a category-level `off`. Unclosed `off` regions
produce `lint/unclosed-suppression` diagnostics.
...
You'll need [ponyc](https://github.com/ponylang/ponyc) and [corral](https://github.com/ponylang/corral) installed.
